...
int main(void)
{
    ...
    unsigned long int period;   // see the comment below.
    ...
    while (1) {
        ...
        IC1CONbits.ICM = 3;     // get rising edge
        beta = getEdgeTime();
        period = beta - alpha;

        if ((period > 32)&&(period << 1599)) //checks and sets RB2
            PORTBbits.RB2 = 1;              //according to freq
