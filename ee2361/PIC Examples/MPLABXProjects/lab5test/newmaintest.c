...
static unsigned int character[16] = {0xF3FF, 0xF097, 0xF037, 0xF043,  //stores the values needed to
                                     0xF27F, 0xF127, 0xF107, 0xF003,  //display characters on PORTB
                                     0xF267, 0xF007, 0xF067, 0xF18B,  //corresponding to each button
                                     0xF07F, 0xF00F, 0xF183, 0xF00B};
...
int main(void)
{
    ...
    LATB = 0xF3FF; //sets output so both 7 seg's are off
    
    unsigned int displayValues[2] = {0xF3FF,0xF3FF}; //stores display values
    unsigned int returned; //stores character read from keypad scan
    unsigned int i;
