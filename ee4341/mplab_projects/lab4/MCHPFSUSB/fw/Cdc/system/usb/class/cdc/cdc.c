...
void USBCheckCDCRequest(void)
{
    ...
    switch(SetupPkt.bRequest)
    {
        case SEND_ENCAPSULATED_COMMAND:
            ...
            wCount.v[0] = 1; //LSB(wCount) = dummy_length;
            break;
        ...
        case GET_LINE_CODING:
            ...
            wCount.v[0] = LINE_CODING_LENGTH; //LSB(wCount) = LINE_CODING_LENGTH;   // Set data count
            break;
        case SET_CONTROL_LINE_STATE:
            ctrl_trf_session_owner = MUID_CDC;
            control_signal_bitmap._byte = SetupPkt.W_Value.v[0]; //LSB(SetupPkt.W_Value);
            break;
...
byte getsUSBUSART(char *buffer, byte len)
{
    ...
    CDC_BULK_BD_OUT.Stat._byte &= _DTSMASK;          /* Save only DTS bit */     \
    CDC_BULK_BD_OUT.Stat.DTS = !CDC_BULK_BD_OUT.Stat.DTS; /* Toggle DTS bit    */     \
    CDC_BULK_BD_OUT.Stat._byte |= _USIE|_DTSEN;      /* Turn ownership to SIE */ \
...
//    mUSBBufferReady(CDC_BULK_BD_IN);
    CDC_BULK_BD_IN.Stat._byte &= _DTSMASK;          /* Save only DTS bit */     \
    CDC_BULK_BD_IN.Stat.DTS = !CDC_BULK_BD_IN.Stat.DTS; /* Toggle DTS bit    */     \
    CDC_BULK_BD_IN.Stat._byte |= _USIE|_DTSEN;      /* Turn ownership to SIE */ \
