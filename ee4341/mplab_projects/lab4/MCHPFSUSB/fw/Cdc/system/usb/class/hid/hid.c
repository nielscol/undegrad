...
//            LSB(wCount) = 1;                    // Set data count
        ...
        case SET_IDLE:
            ctrl_trf_session_owner = MUID_HID;
            idle_rate = SetupPkt.W_Value.v[1]; //MSB(SetupPkt.W_Value);
            break;
        case GET_PROTOCOL:
            ctrl_trf_session_owner = MUID_HID;
...
//            LSB(wCount) = 1;                    // Set data count
        ...
        case SET_PROTOCOL:
            ctrl_trf_session_owner = MUID_HID;
            active_protocol = SetupPkt.W_Value.v[0];//LSB(SetupPkt.W_Value);
            break;
...
//        mUSBBufferReady(HID_BD_OUT);
	    HID_BD_OUT.Stat._byte &= _DTSMASK;          /* Save only DTS bit */     \
    	HID_BD_OUT.Stat.DTS = !HID_BD_OUT.Stat.DTS; /* Toggle DTS bit    */     \
    	HID_BD_OUT.Stat._byte |= _USIE|_DTSEN;      /* Turn ownership to SIE */ \
